...
var mediumJSONFeed = require('../mediumfeed');



var mediumJSONFeedCacheData = {}
var mediumJSONFeedCacheInterval = {}
var cacheItems = [
  '/@gifto/latest?limit=10',
  '/@gifto/latest?limit=20',
  '/@gifto/latest?limit=30',
]

setInterval(function(){
  mediumJSONFeedCacheInterval = {};
},60000)

var mediumJSONFeedWithCache = function(path, callback){
  console.log(path)
  try {
    if(path.split('?').pop().indexOf('&to=') > -1){
      if(mediumJSONFeedCacheData[path])
        return callback(mediumJSONFeedCacheData[path]);
      else
        return mediumJSONFeed(path,function(data){
          mediumJSONFeedCacheData[path] = data;
          callback(data);
        });

    }else if(cacheItems.indexOf(path) > -1 ){
      return mediumJSONFeedCacheInterval[path] 
        && callback(mediumJSONFeedCacheInterval[path])
        || mediumJSONFeed(path,function(data){
          mediumJSONFeedCacheInterval[path] = data;
        callback(data);
      });
    }
  ...
  return mediumJSONFeed(path,callback);
}
...
function withCORS(headers, request) {
  headers['access-control-allow-origin'] = request.headers.host;
...
function getHandler(options, proxy) {
  ...
  return function (req, res) {
    console.log(req.url)
    if (req.url.startsWith('/medium/')) {
      const headers = {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': process.env.ORIGIN || '*'
      };
      return mediumJSONFeedWithCache(req.url.replace('/medium/', '/'), data => {
        res.writeHead(data.status || 500, headers);

        if (data.status == 200) {
          const { status, response = [] } = data
          const compressDatas = response.map(
            ({
              ...
              references,
              previewContent: {
                bodyModel: {paragraphs: previewParagraphs = []} = {},
              } = {},
              content : {
          ...
          res.end(JSON.stringify({ status, response: compressDatas }, null, 2), 'utf-8');
